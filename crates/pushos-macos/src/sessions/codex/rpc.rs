...
async fn deliver(line: &str, waiting: &Waiting) {
    let Some((id, answer)) = read_answer(line) else {
        // A notification, or something a later Codex says that PushOS does
        // not ask about. Both are none of its business.
        return;
    };
