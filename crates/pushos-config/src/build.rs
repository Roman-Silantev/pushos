...
impl RuntimeConfig {
    ...
    pub fn build(file: &ConfigFile) -> Result<Self, ConfigError> {
        ...
        let workflows = crate::workflows::build(file, &mut problems);
        let sequences = crate::sequences::build(file, &mut problems);
        let rest = crate::surface::build(file, &mut problems);
